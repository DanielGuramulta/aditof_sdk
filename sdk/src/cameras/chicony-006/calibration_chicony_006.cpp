...
aditof::Status CalibrationChicony006::SetEEPROMData_0(uint16_t Gdata[][2], uint16_t Gdata_size, uint16_t SetData[])
{
...
aditof::Status CalibrationChicony006::SetEEPROMData_1(uint16_t Gdata[][2], uint16_t Gdata_size, uint16_t SetData[][2])
{
    using namespace aditof;
    Status status = Status::OK;
    for (int i = 0; i < Gdata_size; ++i) {
...
aditof::Status CalibrationChicony006::initialize(std::shared_ptr<aditof::DeviceInterface> device)
{
    ...
    CEEPROM.Data_init(gunCsTable, gunIspRegCmn, gunIspRegMode, gunIspRamMode, gunCtrlCmn);

    for (int i = 0; i < EEPROM_V_size; ++i) TofEepromRead(EEPROM_V_data[i][0], &EEPROM_V_data[i][1], 1);

    bool bread = false;
    bread = CEEPROM.ReadCalEEPROM(EEPROM_V_data, gunCtrlCmn_Get, gunCtrlCmn_Get_size,
        gunIspRegCmn_Get, gunIspRegCmn_Get_size,
        gunCtrlMode_Get, gunCtrlMode_Get_size,
        gunIspRegMode_Get, gunIspRegMode_Get_size,
        gunIspRamMode_Get, gunIspRamMode_Get_size);

    if (!bread) return Status::GENERIC_ERROR;
    ...
    bread = CEEPROM.ReadCSEEPROM(gunCtrlCmn, gunCsTable_Get, gunCsTable_Get_size);
    if (!bread) return Status::GENERIC_ERROR;
    ...
    for (int i = 0; i < PowerUp_data_size; ++i) {
        m_device->writeAfeRegisters(&PowerUp_data[i][0], &PowerUp_data[i][1], 1);
    }
    LOG(INFO) << "**************power up sequence finish";

    /* TOF ISP registers */
    LOG(INFO) << "*******************TOF ISP registers ";
    TofSendCsTable((uint16_t *)gunIspRegCmn, gunIspRegCmn_Size); /* for all mode */
    TofChangeRangeMode(gunRangeMode);                             /* for current mode */
    LOG(INFO) << "*******************TofChangeRangeMode";

    CEEPROM.SetImageFormat(gunRangeMode, Format_data, Format_size, gunIspRegMode);
...
aditof::Status  CalibrationChicony006::TofEepromRead(uint16_t unAddr, uint16_t* punData, uint16_t unSlave)
{
    using namespace aditof;
    Status status = Status::OK;
    uint32_t ADDR_v = uint32_t(unAddr);
    uint8_t             ucData[2];
    m_device->readEeprom(ADDR_v, ucData, 2);
    *punData = ((uint16_t)ucData[0] << 8) | (uint16_t)ucData[1];

    return status;
}

aditof::Status  CalibrationChicony006::TofEepromWrite(uint16_t unAddr, uint16_t punData)
{
    using namespace aditof;
    Status status = Status::OK;
    uint32_t ADDR_v = uint32_t(unAddr);
    ...
    m_device->writeEeprom(ADDR_v, ucData, 2);
...
aditof::Status  CalibrationChicony006::TofSetEmissionEnable(uint16_t unEnable)
{
    ...
    if (unEnable == 1) LOG(WARNING) << "LD Power On";
    else LOG(WARNING) << "LD Power Off";

    return status;
}

aditof::Status CalibrationChicony006::TofSendCsTable(uint16_t *punSndTbl, uint32_t ulWords)
{
    using namespace aditof;
    Status status = Status::OK;
    uint32_t  ulLoop = 0;
...
aditof::Status CalibrationChicony006::unWriteTofRamReg(uint16_t *punReg, uint16_t unReg, uint16_t unSetNum)
{
    using namespace aditof;
    Status status = Status::OK;
    int i;
...
aditof::Status CalibrationChicony006::SetExposureDelay(uint16_t unMode, uint16_t unVdInitOfst)
{
    ...
    unWriteTofRamReg(&gunIspRamMode[unMode][30], unVdIniOfstMinus1, unVdIniOfstAdrNum);

    return status;
}

aditof::Status CalibrationChicony006::unReadTofRamReg(uint16_t *punRegAddr, uint16_t *punReg, uint16_t unRegNum)
{
    using namespace aditof;
    Status status = Status::OK;
...
aditof::Status CalibrationChicony006::GetExposureDelay(uint16_t unMode, uint16_t *punVdInitOfst)
{
    ...
    if (unVdIniOfstAdrNum == 0) {
        *punVdInitOfst = 0;
        ...
        unReadTofRamReg(&gunIspRamMode[unMode][30], &unVdIniOfstMinus1, unVdIniOfstAdrNum);
        *punVdInitOfst = unVdIniOfstMinus1 + 1;
    }
    return status;
}

aditof::Status CalibrationChicony006::GetIdlePeriod(uint16_t unMode, uint16_t *punIdlePeriod)
{
    using namespace aditof;
    Status status = Status::OK;
    ...
    if (unIdlePeriodAdrNum == 0) {
        *punIdlePeriod = 0;
        ...
        unReadTofRamReg(&gunIspRamMode[unMode][69], &unIdlePeriodMinus2, unIdlePeriodAdrNum);
...
aditof::Status CalibrationChicony006::TofSetCcdDummy(uint16_t unCcdDummy)
{
    ...
    CEEPROM.TofSetCcdDummy_data(gunIspRamMode, unCcdDummy, CCD_data, CCD_data_size, gunRangeMode);
    for (int i = 0; i < CCD_data_size; ++i) {
        m_device->writeAfeRegisters(&gunIspRamMode[gunRangeMode][CCD_data[i][0]], &CCD_data[i][1], 1);
    }

    return status;
}

aditof::Status CalibrationChicony006::TofSetExpValue(uint16_t unExp, uint16_t *unHdExp)
{
    using namespace aditof;
    Status status = Status::OK;
    ...
    int   iReadSize2;
    int   iHdExp;
    CEEPROM.TofSetExpValue_Data(unExp, unHdExp, gunRangeMode, gunCtrlMode, gunVdInitialOffset, gunIspRegMode, gunIspRamMode, unLongNum, unShortNum, unLmsNum, gunIdlePeriod, gunCcdDummy, iReadSize2, iHdExp);

    unWriteTofRamReg(&gunIspRamMode[unMode][1], unExp, unLongNum);
    unWriteTofRamReg(&gunIspRamMode[unMode][16], unExp / 4, unShortNum);
    unWriteTofRamReg(&gunIspRamMode[unMode][20], unExp - unExp / 4 - 1, unLmsNum);
...
aditof::Status CalibrationChicony006::TofChangeRangeMode(uint16_t unMode)
{
    ...
    CEEPROM.GetLDFlag(unLdFlag, gunCtrlMode, gunRangeMode);    //////////// Get LD Flag
    ...
    gunExpValue = CEEPROM.TofGetDefExpValue(unMode, gunCtrlMode);//  TofGetDefExpValue(unMode);
    TofSetExpValue(gunExpValue, &gunHdExp);

    uint16_t SE_D[3][2] = { 0 };
    CEEPROM.SetExp_data(SE_D, gunHdExp);
    for (int i = 0; i<3; ++i) m_device->writeAfeRegisters(&SE_D[i][0], &SE_D[i][1], 1);
    ...
    for (int i = 0; i<2; ++i) m_device->writeAfeRegisters(&LD_D[i][0], &LD_D[i][1], 1);
    ...
    if (btal) {
        for (int i = 0; i<2; ++i) m_device->writeAfeRegisters(&TAL_D[i][0], &TAL_D[i][1], 1);
    }
